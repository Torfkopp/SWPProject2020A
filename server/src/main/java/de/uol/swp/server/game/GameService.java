...
public class GameService extends AbstractService {
    ...
    public void onPlayRoadBuildingCardAllowedRequest(PlayRoadBuildingCardAllowedRequest req) {
        ...
        if (!game.getActivePlayer().equals(req.getUser()) || !game.isDiceRolledAlready() || !game
                .isBuildingAllowed() || game.isPaused()) return;
    ...
    private void onAcceptUserTradeRequest(AcceptUserTradeRequest req) {
        ...
        Game game = gameManagement.getGame(req.getOriginLobby());
        if (!game.getActivePlayer().equals(req.getOfferingUser()) || !game.isDiceRolledAlready() || game.isPaused())
            return;
        game.setBuildingAllowed(false);
    ...
    private void onBuildRequest(BuildRequest req) {
        ...
        if (currentPhase == Game.StartUpPhase.NOT_IN_STARTUP_PHASE) {
            if (!game.getActivePlayer().equals(req.getUser()) || !game.isDiceRolledAlready() || game.isPaused()) {
                return;
            }
    ...
    private void onBuyDevelopmentCardRequest(BuyDevelopmentCardRequest req) {
        ...
        Game game = gameManagement.getGame(req.getOriginLobby());
        if (!game.getActivePlayer().equals(req.getUser()) || !game.isDiceRolledAlready() || game.isPaused()) return;
    ...
    private void onTradeWithUserRequest(TradeWithUserRequest req) {
        ...
        if (game.getActivePlayer().equals(req.getUser()) || req.isCounterOffer() || game.isPaused()) {
            game.setBuildingAllowed(false);
            Inventory inventory = game.getInventory(req.getUser());
