...
public class GameMapManagement implements IGameMapManagement {
    ...
    @Override
    public void moveRobber(MapPoint newHex) {
    ...
    public void createBeginnerMap() {
        ...
        //Create settlements
        intersectionMap[1][2].setOwnerAndState(Player.PLAYER_1, SETTLEMENT);
        intersectionMap[3][2].setOwnerAndState(Player.PLAYER_1, SETTLEMENT);
        intersectionMap[1][5].setOwnerAndState(Player.PLAYER_2, SETTLEMENT);
        intersectionMap[4][4].setOwnerAndState(Player.PLAYER_2, SETTLEMENT);
        intersectionMap[2][2].setOwnerAndState(Player.PLAYER_3, SETTLEMENT);
        ...
        //Create roads
        placeRoad(Player.PLAYER_1, edgeConnectingIntersections(intersectionMap[1][2], intersectionMap[1][3]));
        placeRoad(Player.PLAYER_1, edgeConnectingIntersections(intersectionMap[3][2], intersectionMap[3][3]));
        placeRoad(Player.PLAYER_2, edgeConnectingIntersections(intersectionMap[1][5], intersectionMap[1][4]));
        placeRoad(Player.PLAYER_2, edgeConnectingIntersections(intersectionMap[4][4], intersectionMap[4][5]));
        placeRoad(Player.PLAYER_3, edgeConnectingIntersections(intersectionMap[2][2], intersectionMap[2][1]));
        ...
        placeRoad(Player.PLAYER_4, edgeConnectingIntersections(intersectionMap[4][6], intersectionMap[3][6]));
    }

    private void createHexEdgeNetwork() {
        // @formatter:off
        var fancyNetworkBuilder = NetworkBuilder.undirected().allowsParallelEdges(false)
    ...
    private void createIntersectionEdgeNetwork() {
        var secondFancyNetworkBuilder = NetworkBuilder.undirected().allowsParallelEdges(false)
        ...
        for (int y = 0; y < intersectionMap.length; y++) {
            for (int x = 0; x < intersectionMap[y].length; x++) {
                ...
                if (x % 2 == 0 && y == 3) {
                    secondFancyNetworkBuilder.addEdge(intersectionMap[y][x], intersectionMap[y - 1][x], hexEdgeNetwork
                            .edgeConnectingOrNull(hexMap[y][x / 2], hexMap[y][(x / 2) + 1]));
                } else if (x % 2 == 1 && y > 0 && y < 2) {
                    secondFancyNetworkBuilder.addEdge(intersectionMap[y][x], intersectionMap[y - 1][x - 1],
                                                      hexEdgeNetwork.edgeConnectingOrNull(hexMap[y][(x - 1) / 2],
                                                                                          hexMap[y][(x + 1) / 2]));
                } else if (x % 2 == 0 && y > 3) {
                    secondFancyNetworkBuilder.addEdge(intersectionMap[y][x], intersectionMap[y - 1][x + 1],
                ...
                if (x % 2 == 1 && y < 3) {
                    secondFancyNetworkBuilder.addEdge(intersectionMap[y][x], //
                                                      intersectionMap[y][x - 1], //
                                                      hexEdgeNetwork.edgeConnectingOrNull(hexMap[y][(x - 1) / 2],
                                                                                          hexMap[y + 1][(x + 1) / 2]));
                } else if (y >= 3 && x % 2 == 0 && x > 1) {
                    secondFancyNetworkBuilder.addEdge(intersectionMap[y][x], intersectionMap[y][x - 1], hexEdgeNetwork
                            .edgeConnectingOrNull(hexMap[y][x / 2], hexMap[y + 1][x / 2]));
                }
                ...
                if ((x % 2 == 0 && x > 0 && y < 3)) {
                    ...
                    secondFancyNetworkBuilder.addEdge(intersectionMap[y][x], intersectionMap[y][x - 1], hexEdgeNetwork
                            .edgeConnectingOrNull(hexMap[y][(x / 2) + 1], hexMap[y + 1][x / 2]));
                }
            }
        }
